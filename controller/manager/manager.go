...
type (
	...
	Manager interface {
		...
		Services() ([]*shipyard.ServiceInfo, error)                                                 //===new
...
func (m DefaultManager) init() error {
	...
	// 创建目录存储编排文件
	err := os.MkdirAll(ContainerDir, 0600)
	if err != nil {
		fmt.Println(err)
	}
	return nil
...
func (m DefaultManager) saveNodeGeneralInfo(nodes []*shipyard.Node) error {
	//存储节点的简要信息
	...
	for i := range(nodes) {
		...
		//fmt.Println(sysInfo)
		if err != nil {
			log.Errorf("err get node general info (%s) : %s", "", err)
	...
	if err != nil {
		log.Errorf("err delete node general info: %s",  err)
	}
	_, err = r.Table(tblNameNodesGeneralInfo).Insert(ngis).RunWrite(m.session)
	if err != nil {
		log.Errorf("err insert node general info: %s",  err)
	}
	return nil
	//fmt.Println(ngis)
}
...
func (m DefaultManager) saveImageInfo(addrs []string){
	...
	if err != nil {
		log.Errorf("error delete image info: %s", err)
	}
	_, err = r.Table(tblNameImageInfo).Insert(rethinkImageInfo).RunWrite(m.session)
	if err != nil {
		log.Warnf("err save image info: %s", err)
	}
...
func (m DefaultManager) saveSysInfo(addr string, tag string){
	...
	case "host":
		...
		if err != nil {
			log.Errorf("error delete node (%s) sysinfo: %s", str, err)
		}
...
func (m DefaultManager) SaveYmlConfigFile(rd io.Reader, boundary string, path string, node string) (string, error) {
	...
	for i := range containerName {
		...
		if res.isNil() && err == nil {
			if err := res.One(ymlConfig.Id); err != nil {
...
func (m DefaultManager) Services() ([]*shipyard.ServiceInfo, error) {
	res, err := r.Table(tblNameServices).Pluck("id", "name", "description", "servicetype", "hosts", "status", "containers").Run(m.session)
	if err != nil {
		return nil, err
	}
...
func (m DefaultManager) GetHostInfo(addr string) (*shipyard.RespHostInfo, error) {
	...
	addr = string(b)
	res ,err := r.Table(tblNameHostInfo).Filter(map[string]string{"node": addr}).Run(m.session)
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (m DefaultManager) GetMemInfo(addr string) (*shipyard.RespMemInfo, error) {
	...
	res ,err := r.Table(tblNameMemInfo).Filter(map[string]string{"node": addr}).Run(m.session)
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (m DefaultManager) GetNetInfo(addr string) (*shipyard.RespNetInfo, error) {
	...
	res ,err := r.Table(tblNameNetInfo).Filter(map[string]string{"node": addr}).Run(m.session)
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (m DefaultManager) GetLoadInfo(addr string) (*shipyard.RespLoadInfo, error) {
	...
	res ,err := r.Table(tblNameLoadInfo).Filter(map[string]string{"node": addr}).Run(m.session)
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (m DefaultManager) GetDiskInfo(addr string) (*shipyard.RespDiskInfo, error) {
	...
	res ,err := r.Table(tblNameDiskInfo).Filter(map[string]string{"node": addr}).Run(m.session)
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (m DefaultManager) GetCpuInfo(addr string) (*shipyard.RespCpuInfo, error) {
	...
	res ,err := r.Table(tblNameCpuInfo).Filter(map[string]string{"node": addr}).Run(m.session)
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (m DefaultManager) GetUserInfo(addr string) (*shipyard.RespUserInfo, error) {
	...
	res ,err := r.Table(tblNameUserInfo).Filter(map[string]string{"node": addr}).Run(m.session)
	if err != nil {
		return nil, err
	}
	defer res.Close()
